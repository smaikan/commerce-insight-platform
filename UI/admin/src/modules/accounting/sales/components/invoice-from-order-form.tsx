...
export function InvoiceFromOrderForm({ orderId, orderNumber, status }: { orderId: string; orderNumber: string; status: number }) {
  ...
  // Burada fatura oluşturma sonucunda yönlendirme ve yerinde yenilemeden yalnız birini çalıştırıyorum.
  useEffect(() => { if (state.redirectHref) router.replace(state.redirectHref); else if (state.refresh) router.refresh(); guard.current = false; }, [router, state]);
