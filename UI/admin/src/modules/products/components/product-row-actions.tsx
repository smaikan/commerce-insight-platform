...
export function ProductRowActions({ id, title, status }: { id: string; title: string; status: ProductStatus }) {
  ...
  return (
    <div className="relative inline-flex">
      <button ref={triggerRef} type="button" onClick={toggleMenu} aria-haspopup="menu" aria-expanded={open} aria-label={`${title} için işlemler`} className="inline-flex size-8 items-center justify-center rounded-lg border border-transparent text-muted transition-colors hover:border-border hover:bg-surface-strong hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">
      ...
      {open ? (
        <div ref={menuRef} role="menu" style={{ top: position.top, right: position.right }} className="fixed z-[70] w-52 overflow-hidden rounded-lg border border-border bg-surface-strong p-1.5 text-left shadow-xl">
          <Link href={`/products/${encodeURIComponent(id)}`} role="menuitem" className="flex min-h-9 items-center rounded-md px-3 text-sm font-semibold text-foreground hover:bg-surface-subtle">Ürünü düzenle</Link>
          {quickStatus !== null ? <button type="button" role="menuitem" disabled={pending} onClick={changeStatus} className="flex min-h-9 w-full items-center rounded-md px-3 text-sm font-semibold text-foreground hover:bg-surface-subtle disabled:opacity-60">{quickStatus === 1 ? "Ürünü aktifleştir" : "Taslak olarak kaydet"}</button> : null}
          <div className="my-1 border-t border-border" />
          <button type="button" role="menuitem" onClick={() => { setOpen(false); setConfirmDelete(true); }} className="flex min-h-9 w-full items-center rounded-md px-3 text-sm font-semibold text-danger hover:bg-danger/10">Ürünü sil</button>
        </div>
      ) : null}
