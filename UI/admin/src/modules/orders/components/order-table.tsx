    ...
    <div className="overflow-x-auto bg-surface-strong">
      <table className="w-full min-w-[920px] border-collapse text-left text-sm">
        <thead className="border-b border-border bg-surface-subtle/80 text-[11px] font-bold uppercase tracking-[0.08em] text-muted">
          <tr>
            <th scope="col" className="w-[32%] px-4 py-2.5">Müşteri / sipariş</th>
            <th scope="col" className="px-3 py-2.5">Durum</th>
            <th scope="col" className="px-3 py-2.5">İçerik</th>
            <th scope="col" className="px-3 py-2.5 text-right">Toplam</th>
            <th scope="col" className="px-3 py-2.5">Oluşturulma / ödeme</th>
            <th scope="col" className="w-32 px-3 py-2.5 text-right">Hızlı bakış</th>
          </tr>
        </thead>
            ...
            return (
              <OrderExpandableRows key={order.id} orderId={order.id} orderNumber={order.orderNumber} orderHref={orderHref}>
                <td className="px-4 py-2.5">
                  <Link href={orderHref} className="flex min-w-0 items-center gap-2.5 rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2">
                    <span className="flex size-9 shrink-0 items-center justify-center rounded-lg border border-primary/15 bg-primary-soft/45 text-primary">
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-0.5 block truncate text-xs font-medium text-muted">Sipariş no: {order.orderNumber}</span>
                    </span>
                  </Link>
                </td>
