  ...
  return (
    <div className="flex flex-col items-end gap-1.5">
      <form action={formAction}>
        <button type="submit" disabled={pending} aria-label={`${deviceName} oturumunu sonlandır`} className="min-h-9 rounded-lg border border-border-strong bg-surface-strong px-3 text-xs font-semibold text-foreground hover:bg-surface-subtle disabled:cursor-not-allowed disabled:opacity-60">{pending ? "Sonlandırılıyor…" : "Oturumu sonlandır"}</button>
      </form>
...
export function RevokeAllSessionsDialog({ sessionCount }: { sessionCount: number }) {
  ...
  return (
    <>
      <button type="button" onClick={() => dialogRef.current?.showModal()} className="inline-flex min-h-10 items-center justify-center rounded-lg border border-danger/40 bg-surface-strong px-4 text-sm font-semibold text-danger hover:bg-danger/5">Tüm oturumları kapat</button>
      <dialog ref={dialogRef} aria-labelledby="revoke-all-title" aria-describedby="revoke-all-description" className="m-auto w-[min(92vw,30rem)] rounded-xl border border-border bg-surface p-0 text-foreground shadow-xl backdrop:bg-foreground/45">
        <form action={formAction} className="p-5">
          ...
          <div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button type="button" onClick={() => dialogRef.current?.close()} disabled={pending} className="min-h-10 rounded-lg border border-border-strong bg-surface-strong px-4 text-sm font-semibold text-foreground hover:bg-surface-subtle">Vazgeç</button>
            <button type="submit" disabled={pending} className="min-h-10 rounded-lg bg-danger px-4 text-sm font-semibold text-white hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-60">{pending ? "Oturumlar kapatılıyor…" : "Tümünü kapat ve çıkış yap"}</button>
          </div>
        </form>
      </dialog>
    </>
