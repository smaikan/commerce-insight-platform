...
export function CatalogPagination({
  ...
  return (
    <nav className="mt-12 flex items-center justify-between border-t border-line pt-6" aria-label="Ürün sayfaları">
      {page > 1 ? (
        <Link className="focus-ring rounded-lg border border-line bg-surface px-4 py-2.5 text-sm font-semibold hover:border-brand-600" href={catalogHref({ ...view, page: page - 1 })} rel="prev">
          Önceki
        </Link>
      ) : <span />}
      <p className="text-sm text-ink-muted"><span className="font-semibold text-ink">{page}</span> / {totalPages}</p>
      {page < totalPages ? (
        <Link className="focus-ring rounded-lg border border-line bg-surface px-4 py-2.5 text-sm font-semibold hover:border-brand-600" href={catalogHref({ ...view, page: page + 1 })} rel="next">
          Sonraki
        </Link>
      ) : <span />}
    </nav>
