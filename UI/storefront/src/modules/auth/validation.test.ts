...
describe("auth form validation", () => {
  ...
  // Burada kayıt sınırlarının eksik ad, kısa şifre ve eşleşmeyen doğrulamayı ayrı alanlarda bildirdiğini doğruluyorum.
  it("returns field-level register errors without retaining passwords", () => {
    ...
    if (!result.success) {
      expect(result.errors).toMatchObject({ firstName: expect.any(String), lastName: expect.any(String), email: expect.any(String), password: expect.any(String), confirmPassword: expect.any(String) });
